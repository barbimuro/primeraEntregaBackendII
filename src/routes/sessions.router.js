...
sessionsRouter.post('/register', (req, res, next) => {
    passport.authenticate('register', async (err, user, info) => {
        ...
        const userToken = jwt.sign(userSession, 'barbiSecreto', { expiresIn: "1d" });
        console.log('Setting cookie with token:', userToken);
        res.cookie('tokenEspecial', userToken, { httpOnly: true }).redirect('/profile');
    })(req, res, next);
});
...
sessionsRouter.post('/login', (req, res, next) => {
    passport.authenticate('login', async (err, user, info) => {
        ...
        const userToken = jwt.sign(userSession, 'barbiSecreto', { expiresIn: "1d" });
        res.cookie('tokenEspecial', userToken).redirect('/profile');
    })(req, res, next);
});
...
sessionsRouter.get('/current', (req, res) => {
    const token = req.cookies['tokenEspecial'];
    if (!token) {
        return res.status(401).send({ status: "error", error: "Please log in" });
    }
    try {
        const user = jwt.verify(token, 'barbiSecreto');
        res.send(user);
    } catch (error) {
...
sessionsRouter.get('/logout', (req, res) => {
    res.clearCookie('tokenEspecial').redirect('/login');
});
